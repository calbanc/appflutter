...
class _controlAccesoFormState extends State<controlAccesoForm> {
  ...
  Widget build(BuildContext context) {
    ...
    const WidgetStateProperty<Color?> trackColor =
      ...
      <WidgetStatesConstraint, Color>{WidgetState.selected: Colors.blue},
    );

    final WidgetStateProperty<Color?> overlayColor =
        WidgetStateProperty<Color?>.fromMap(
      <WidgetState, Color>{
        WidgetState.selected: Colors.blue.withOpacity(0.54),
          ...
          Padding(
            ...
            child: GestureDetector(
              onTap: () async {
                ...
                String nombrefotoguia = fecha.replaceAll("/", "") +
                    hora.replaceAll(":", "") +
                    'guia';
                String nombrefotofactura = fecha.replaceAll("/", "") +
                    hora.replaceAll(":", "") +
                    'fact';
                String nombrefotoproducto = fecha.replaceAll("/", "") +
                    hora.replaceAll(":", "") +
                    'pro';
                String nombrefototransporte = fecha.replaceAll("/", "") +
                    hora.replaceAll(":", "") +
                    'trans';
                ...
                http.Response respuesta =
                    await widget.provider.saveacceso(widget.provider);
                ...
                if (respuesta.statusCode == 200) {
                  ...
                  widget.provider.imagepathguia != ''
                      ? await widget.provider.uploadImage(
                          widget.provider.imagepathguia, nombrefotoguia)
                      : null;
                  widget.provider.imagepathfactura != ''
                      ? await widget.provider.uploadImage(
                          widget.provider.imagepathfactura, nombrefotofactura)
                      : null;
                  widget.provider.imagepathproducto != ''
                      ? await widget.provider.uploadImage(
                          widget.provider.imagepathproducto, nombrefotoproducto)
                      : null;
                  widget.provider.imagepathproducto != ''
                      ? await widget.provider.uploadImage(
          ...
          child: Column(
            ...
            children: [
                        ...
                        return Padding(
                          ...
                          child: DropdownSearch<Client>(
                            ...
                            onChanged: (Client? value) async {
                              http.Response respuesta = await widget.provider
                                  .getzonbyclients(value!.id);
                              if (respuesta.statusCode == 200) {
                                ZonebyClientResponse response =
                                    ZonebyClientResponse.fromJson(
                                        respuesta.body);
              ...
              Padding(
                padding: EdgeInsets.symmetric(vertical: 10, horizontal: 18),
                child: TextFormField(
                  controller: widget.provider.empresactrl,
                  decoration: InputDecoration(
                      labelText: 'Empresa de visita',
                      // Set border for enabled state (default)
                      enabledBorder: OutlineInputBorder(
                        borderSide:
                            const BorderSide(width: 1, color: Colors.black),
